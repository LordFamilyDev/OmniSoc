...
//
// Mirror of CPP_OmniSoc/include/PackBytes.h. Kept as a copy because the
// Arduino IDE does not follow `..` paths into sibling library folders.
...
inline const uint8_t* unpack_i8 (const uint8_t* p, int8_t*  out) { uint8_t  t; const uint8_t* r = unpack_u8 (p, &t); *out = (int8_t) t; return r; }
inline const uint8_t* unpack_i16(const uint8_t* p, int16_t* out) { uint16_t t; const uint8_t* r = unpack_u16(p, &t); *out = (int16_t)t; return r; }
inline const uint8_t* unpack_i32(const uint8_t* p, int32_t* out) { uint32_t t; const uint8_t* r = unpack_u32(p, &t); *out = (int32_t)t; return r; }
inline const uint8_t* unpack_i64(const uint8_t* p, int64_t* out) { uint64_t t; const uint8_t* r = unpack_u64(p, &t); *out = (int64_t)t; return r; }
